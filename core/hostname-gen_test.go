...
func TestExpandHostNames(t *testing.T) {
	// SINGLE IP
	input := "192.168.0.1"
	hosts, err := ExpandHostNames(input)
	test.CheckErr(t, err)
	...
	// // TODO: file + kubectl get nodes
	// // TODO: shell: window bash zsh sh
	...
	// Malformed ranges
	_, err = ExpandHostNames("192.[2:1].0")
	test.IsError(t, err)

	// _, err = ExpandHostNames("192.[0].0")
	// test.IsError(t, err)

	// _, err = ExpandHostNames("192.[0.0")
	// test.IsError(t, err)

	// _, err = ExpandHostNames("192.[1:2:1:2]")
	// test.IsError(t, err)
}
