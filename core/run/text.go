...
// TextTUI runs tasks and writes output to provided writers instead of os.Stdout/os.Stderr
func (run *Run) TextTUI(dryRun bool, stdout io.Writer, stderr io.Writer) (dao.ReportData, error) {
	task := run.Task
	servers := run.Servers
	uServers := run.UnreachableServers
	...
	switch task.Spec.Strategy {
	case "free":
		err = run.freeTextTUI(prefixMaxLen, reportData, dryRun, stdout, stderr)
	case "host_pinned":
		err = run.hostPinnedTextTUI(prefixMaxLen, reportData, dryRun, stdout, stderr)
	default: // linear
		err = run.linearTextTUI(prefixMaxLen, reportData, dryRun, stdout, stderr)
	}
...
func (run *Run) linearTextTUI(
	...
	for t := 0; t < taskLen; t++ {
		...
		if run.Task.Theme.Text.Header != "" {
			...
			err := printTaskHeaderTUI(t, taskLen, run.Task.Tasks[t].Name, run.Task.Tasks[t].Desc, run.Task.Theme.Text, stdout)
			if err != nil {
				return err
			}
			fmt.Fprintln(stdout)
		...
		for k := 0; k < quotient; k++ {
			...
			for _, r := range runs[start:end] {
				...
				go func(
					...
					err := run.textWorkTUI(r, 0, register, prefixMaxLen, reportData, dryRun, batch, stdout, stderr)
					<-waitChan
					if err != nil {
						errCh <- err
						failedHostsCh <- struct {
	...
	for k := 0; k < quotient; k++ {
		...
		for i := range runs[start:end] {
			...
			go func(
				...
				mu.Lock()
				err := run.textWorkTUI(r, r.j, register, prefixMaxLen, reportData, dryRun, batch, stdout, stderr)
				mu.Unlock()
...
func (run *Run) hostPinnedTextTUI(
	prefixMaxLen int,
	reportData dao.ReportData,
	dryRun bool,
	stdout io.Writer,
			...
			go func(
				...
				for i, j := range r {
					...
					if run.Task.Theme.Text.Header != "" && batch == 1 {
						mu.Lock()
						fmt.Fprintln(stdout)
						err := printTaskHeaderTUI(i, taskLen, j.Cmd.Name, j.Cmd.Desc, run.Task.Theme.Text, stdout)
						fmt.Fprintln(stdout)
						mu.Unlock()
						if err != nil {
							<-waitChan
					...
					err := run.textWorkTUI(j, 0, register[j.Server.Name], prefixMaxLen, reportData, dryRun, batch, stdout, stderr)
					<-waitChan
					if err != nil {
						errCh <- err
						failedHosts <- true
...
func (run *Run) textWorkTUI(
	...
	start := time.Now()
	var wg sync.WaitGroup
	out, stdoutStr, stderrStr, err := runTextCmdTUI(si, t, prefix, r.Cmd.Register, &wg, stdout, stderr)
	reportData.Tasks[r.i].Rows[r.j].Duration = time.Since(start)
	...
	if r.Cmd.Register != "" {
		register[r.Cmd.Register] = strings.TrimSuffix(out, "\n")
		register[r.Cmd.Register+"_stdout"] = stdoutStr
		register[r.Cmd.Register+"_stderr"] = stderrStr
		register[r.Cmd.Register+"_rc"] = fmt.Sprint(reportData.Tasks[t.rIndex].Rows[r.j].ReturnCode)
		if err != nil {
			register[r.Cmd.Register+"_failed"] = "true"
			if r.Task.Spec.IgnoreErrors || r.Cmd.IgnoreErrors {
...
func runTextCmdTUI(
	...
	go func(client Client) {
		...
		if register == "" {
			if t.print != "stderr" {
				if prefix != "" {
					_, err = io.Copy(stdout, core.NewPrefixer(client.Stdout(i), prefix))
				} else {
					_, err = io.Copy(stdout, client.Stdout(i))
				}
			}
		} else {
			if t.print != "stderr" {
				...
				r := io.TeeReader(client.Stdout(i), mw)
				if prefix != "" {
					_, err = io.Copy(stdout, core.NewPrefixer(r, prefix))
				} else {
	...
	go func(client Client) {
		...
		if register == "" {
			if t.print != "stdout" {
				if prefix != "" {
					_, err = io.Copy(stderr, core.NewPrefixer(client.Stderr(i), prefix))
				} else {
					_, err = io.Copy(stderr, client.Stderr(i))
				}
			}
		} else {
			if t.print != "stdout" {
				...
				r := io.TeeReader(client.Stderr(i), mw)
				if prefix != "" {
					_, err = io.Copy(stderr, core.NewPrefixer(r, prefix))
				} else {
...
func printTaskHeaderTUI(i int, numTasks int, name string, desc string, ts dao.Text, w io.Writer) error {
	data := HeaderData{
		Name:     name,
		Desc:     desc,
		Index:    i + 1,
...
func printCmdTUI(prefix string, cmd string, w io.Writer) {
	scanner := bufio.NewScanner(strings.NewReader(cmd))
	for scanner.Scan() {
		fmt.Fprintf(w, "%s%s\n", prefix, scanner.Text())
	}
